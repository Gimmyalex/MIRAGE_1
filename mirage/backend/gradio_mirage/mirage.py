...
class MirageMessage(GradioModel):
    text: Optional[str] = None
    files: Optional[List[FileMessage]] = None


class ChatbotData(GradioRootModel):
    root: List[Tuple[Optional[MirageMessage], Optional[MirageMessage]]]


class Mirage(Component):
    """
    ...
    Also supports audio/video/image files, which are displayed in the Mirage, and other kinds of files which are displayed as links. This
    ...
    def __init__(
        ...
        *,
        ...
        Parameters:
            ...
            placeholder: a placeholder message to display in the chatbot when it is empty. Centered vertically and horizontally in the Mirage. Supports Markdown and HTML. If None, no placeholder is displayed.
        """
    ...
    def _postprocess_chat_messages(
        self, chat_message: MirageMessage | dict | None
    ) -> MirageMessage | FileMessage | None:
        if chat_message is None:
            return None
        if isinstance(chat_message, dict):
            chat_message = MirageMessage(**chat_message)
    ...
    def postprocess(
        ...
        """
        Parameters:
            value: expects a `list[list[str | None | tuple]]`, i.e. a list of lists. The inner list should have 2 elements: the user message and the response message. The individual messages can be (1) strings in valid Markdown, (2) tuples if sending files: (a filepath or URL to a file, [optional string alt text]) -- if the file is image/video/audio, it is displayed in the Mirage, or (3) None, in which case the message is not displayed.
        Returns:
